...
export default class editPicture extends Component {
    constructor(props) {
        ...
        this.path_count = this.path_count.bind();
    }
    ...
    grabPixels = async (success, path) => {
        ...
        AsyncStorage.setItem("pathsForAcne", JSON.stringify(this.state.data));
        ...
        console.log("save start....", finalPath)
        ...
        fetch('http://18.222.228.44:3000/acneImage', dataFetch)
            .then((response) => {
                return response;
            })
            ...
            .then(responseJson => {
                ...
                if (responseJson.status === "true") {
                    ...
                    const check = user_id == null ? JSON.stringify(responseJson.response.insertId) : user_id;
                    AsyncStorage.setItem("user_id", check);

                    this.props.navigation.navigate('fineLines');
                } else {
    ...
    path_count = async (pathsCount) => {
        ...
        this.setState({
            acnePathCount: pathsCount
        });
        AsyncStorage.setItem("pathAcne", JSON.stringify(pathsCount));
    }
                ...
                <View style={{backgroundColor: '#DFE6EC'}}>
                    <RNSketchCanvas
                        ...
                        savePreference={() => {
                            return {
                                folder: 'StopGuessing',
                                filename: String("Acne-" + this.state.user_id + "-" + Math.ceil(Math.random() * 100000000)),
                                transparent: true,
                                imageType: 'jpg',
                                includeImage: true,
                                includeText: false,
