...
export default class fineLines extends Component {
    constructor(props) {
        super(props);
        this.state = {
            spinner: false,
    ...
    check = async (success, path) => {
        AsyncStorage.setItem("pathFineLines", JSON.stringify(this.state.pathcountFine));
        if (this.state.pathcountFine === 0 && Platform.OS !== 'ios') {
    ...
    simpleImage = async () => {
        ...
        const finalPath = await AsyncStorage.getItem('thankYouImage');
        const newVar = finalPath.split("/");

        const imgMame = newVar[newVar.length - 1];
        ...
        fetch('http://18.222.228.44:3000/simpleImage', dataFetch)
    ...
    grabPixels = async (success, path) => {
        ...
        AsyncStorage.setItem("pathsForFineLines", JSON.stringify(this.state.data));
        await this.simpleImage();
        ...
        fetch('http://18.222.228.44:3000/fineLines', dataFetch)
            .then(function (response) {
                return response;
            })
            .then(response => response.json())
    ...
    path_count = async (pathsCount) => {
        if(this.state.loading) {
            ...
            this.props.navigation.navigate('editPicture');
        ...
        this.setState({pathcountFine: pathsCount});
    }
                ...
                <View style={{backgroundColor: '#DFE6EC'}}>
                    <RNSketchCanvas
                        ...
                        clearComponent={<View style={Platform.OS === "ios" ? styles.clear : styles.clearAndroid}>
                            ...
                            }}>{this.state.data.length > 0 ? "Clear" : "Back"}</Text>
                        </View>}
                        ...
                        savePreference={() => {
                            return {
                                folder: 'StopGuessing',
                                filename: String("FineLines-" + this.state.user_id + "-" + Math.ceil(Math.random() * 100000000)),
                                transparent: true,
                                imageType: 'jpg',
                                includeImage: true,
                                includeText: false,
                ...
                <View style={{bottom: 102}}>
                    <View style={{
                        ...
                        <Text style={{
                            ...
                            marginTop: 10,
                            fontFamily: 'Khula-Bold'
                        }}>Touch the Fine Lines</Text></View>
