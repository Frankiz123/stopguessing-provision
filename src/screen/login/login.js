  ...
  log_in = async () => {
    ...
    fetch('http://18.222.228.44:3000/signin', datafetch)
      ...
      .then(responseJson => {
        ...
        console.log ("login res....", responseJson)
        if (responseJson.status == "true") {
          AsyncStorage.setItem("responseJson",JSON.stringify(responseJson.user[0]))
          this.setState({
            spinner: false,
          });
          AsyncStorage.removeItem('logout_token');
          if (subscribe_id == null) {
            this.props.navigation.navigate("Tutorial");

          }
          else {
            AsyncStorage.setItem("loginToken",JSON.stringify('true'));
...
newUser = async() =>
{
  AsyncStorage.setItem("newUserToken",JSON.stringify(1))
  this.props.navigation.navigate('Tutorial');
}
